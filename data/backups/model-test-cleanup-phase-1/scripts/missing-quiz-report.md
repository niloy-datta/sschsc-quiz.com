...
| Category | In public/quiz-data | UI route | Notes |
| -------- | ------------------- | -------- | ----- |
...
| Live test | **No static question set** | /live-test | Mock countdown UI only |

## Detailed Missing / Low Data List
...
### Low Model Test Count (< 5 sets)
- None

### Missing Whole Syllabus Tests
- All subjects: no `full-book` / `whole-syllabus` keys in `public/quiz-data` JSON
- UI routes `/ssc/full-book-test` and `/hsc/full-book-test` show **Coming Soon**

### Live Test
- /live-test page exists; uses mock countdown â€” no real live question set in static JSON
...
- `à¦ªà§à¦°à¦¶à§à¦¨ à¦à¦–à¦¨à§‹ à¦¯à§‹à¦— à¦•à¦°à¦¾ à¦¹à§Ÿà¦¨à¦¿à¥¤` â€” used when chapter/model list is empty
- `à¦¶à§€à¦˜à§à¦°à¦‡ à¦†à¦¸à¦›à§‡à¥¤` â€” used on full-book-test Coming Soon pages
- Model test pages show only real sets; no fake cards