/**
 * Import SSC Chemistry premium chapter-wise JSON (chapters[].sets[] format).
 *
 * Usage:
 *   node scripts/import-ssc-chemistry-premium.js [path-to-json]
 *
 * Default source: data/imports/ssc-chemistry-premium-full.json
 */
const fs = require("fs");
...
function convertQuestion(q, setId, chapterNo, chapterName) {
...
function buildSetSlug(chapterNo, setNo) {
  return `ssc-chemistry-chapter-${chapterNo}-model-test-${String(setNo).padStart(2, "0")}`;
}

function importPremium(sourcePath) {
  ...
  const megaPath = path.join(PUBLIC, "quiz-data", LEVEL, `${SUBJECT_SLUG}.json`);
  ...
  const modelIndexPath = path.join(PUBLIC, "quiz-data", LEVEL, `${SUBJECT_SLUG}.model-tests.index.json`);
  let modelIndex = null;
  ...
  for (const ch of chapters) {
    ...
    for (const set of ch.sets ?? []) {
      ...
      const slug = buildSetSlug(chapterNo, setNo);
      const existed = Boolean(mega.modelTests[slug]);

      const converted = set.questions.map((q) =>
        convertQuestion(q, slug, chapterNo, chapterName),
      );
      ...
      const pubPath = path.join(PUBLIC, "questions", SUBJECT_SLUG, `${slug}.json`);
      const priPath = path.join(BACKEND_ANSWERS, SUBJECT_SLUG, `${slug}.answers.json`);
...
const sourcePath = process.argv[2] || "data/imports/ssc-chemistry-premium-full.json";
importPremium(sourcePath);
